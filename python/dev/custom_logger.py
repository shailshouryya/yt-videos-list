...
def log_extraction_information(module, writer_function, args, kwargs):
    ...
    file_name, videos_written, reverse_chronological, logging_output_location = writer_function(*args, **kwargs)   # writer_function() writes to temp_{file_name}
    if module.endswith('create_file'): log(f'Opening a temp {extension} file and writing video information to the file....',           logging_output_location)
    if module.endswith('update_file'): log(f'Opening a temp {extension} file and writing ***NEW*** video information to the file....', logging_output_location)
    ...
    if module.endswith('create_file'): log(f'{videos_written} videos written to'.ljust(39) + f'{temp_file}',           logging_output_location)
    if module.endswith('update_file'): log(f'{videos_written} ***NEW*** videos written to'.ljust(39) + f'{temp_file}', logging_output_location)
    ...
    if module.endswith('create_file'): log(f'It took {total_time} seconds to write all {videos_written} videos to {final_file}{NEWLINE}',                             logging_output_location)
    if module.endswith('update_file'): log(f'It took {total_time} seconds to write the {videos_written} ***NEW*** videos to the pre-existing {final_file} {NEWLINE}', logging_output_location)
